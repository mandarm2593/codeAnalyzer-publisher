...
inline void CodePublisher::CreateHTMLFiles(std::unordered_map<std::string, std::set<std::string>> deptable,std::string x) {
	...
	homepage << "\n <a href =\"" + cssloc + "\"><label>styling.css</label></a>,<a href = \"" +jssloc + "\"><label>jshidebutton.js</label> </a></head><body><br><br> List of TA test files and important packages : <pre><br>";
